...
function chaotic-aur {
	...
	cont 
	    ...
	    artix-chroot /mnt bash -c "pacman-key --recv-key 3056513887B78AEB --keyserver keyserver.ubuntu.com && pacman-key --lsign-key 3056513887B78AEB && exit" 
		...
		artix-chroot /mnt bash -c "echo '
		[chaotic-aur]' >> /etc/pacman.conf && echo '
		Include = /etc/pacman.d/chaotic-mirrorlist' >> /etc/pacman.conf && pacman -Sy && exit"
	fi
	cont
}
